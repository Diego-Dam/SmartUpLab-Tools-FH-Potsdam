...
// display related variables
	...
	init {
		...
		string QUERY_people <- "SELECT  block, Personal_ID, age, household_category,
									today_activity_plan, household, working_age,
...
// The bus manager is an agent that have all departure times for a given line and create a bus at the given moment
        ...
        if include_new_bus_line = true {
	        rdt_departure_csv <- csv_file("C:/Users/diego/python-workspace/drt_route_planning_app_local/apps/output/" + scenario_name + "_rdt_timetable.csv", ',', string, true);
...
species rdt_bus skills: [advanced_driving] frequency: 1 control: fsm {
		...
		// if both route are done
			...
			ask DB_accessor {
				do insert (params: PARAMS, into: "results_" + scenario_name + "_rdt_data" , 
	               columns: ["start_trip_time", "trip_duration", "passengers_by_stop"], 
	               values: [myself.start_trip_time, myself.trip_duration, myself.passengers_by_stop]);
			}
			if (current_road != nil) {
				ask road(current_road) {
					do unregister(myself);
	...
	state driving initial:false {
		...
		if real_speed = 0 and state = 'driving' and current_road != nil and current_path != nil {
			...
			list<road> test <- (driving_roads at_distance 200);
				loop i over: test {
					location <- i.location;
					current_path <- compute_path(graph: road_network, target: self.bus_target);
	...
	state passengers_get_on_off initial:false {
		...
		if !(empty(passengers_getting_on)) {
			...
			write "Rdt bus " + self + " has already " length(members) + " passengers and " + length(passengers_getting_on) + " that want to get in for a maximal capacity of " + rdt_capacity;
			if length(members) + length(passengers_getting_on)>rdt_capacity {
				int n_can_get_on <- rdt_capacity - length(members);
				list<people> can_get_on <- first(n_can_get_on, passengers_getting_on);
				passengers_getting_on <- nil;
				loop i over: can_get_on {
...
experiment road_traffic type: gui {
    ...
    parameter "scenario_name" category: "Output-Displays" var: scenario_name <- "classic_bornbus";
    // parameter "Percentage home service" category: "Demand" var: perc_pick_up_service init: 50 min: 0 max: 100 step: 1;
    parameter "people_df" category: "Output-Displays" var: people_df <- "people_400_random" among: ["test_people3", "people_2306", "people_400_random"];
